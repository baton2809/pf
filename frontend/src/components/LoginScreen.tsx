...
export const LoginScreen: React.FC = () => {
  const { loginWithEmail, loginWithGoogle, isBackendAvailable } = useAuth();
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      ...
      const mode = isBackendAvailable ? 'через API' : 'в демо режиме';
      showNotification(`Успешный вход в систему ${mode}`, 'success');
    } catch (error) {
  ...
  const handleGoogleLogin = () => {
    // mock google authorization - directly log in
    loginWithGoogle();
  };
  ...
  const handleQuickRegister = async () => {
    setIsLoading(true);
    try {
      // Используем моковые данные для быстрой регистрации
      await loginWithEmail('demo@example.com');
      const mode = isBackendAvailable ? 'через API' : 'в демо режиме';
      showNotification(`Успешная регистрация ${mode}`, 'success');
    } catch (error) {
  ...
  const handleForgotPassword = async () => {
    setIsLoading(true);
    try {
      // Используем моковые данные для восстановления пароля
      await loginWithEmail('reset@example.com');
      const mode = isBackendAvailable ? 'через API' : 'в демо режиме';
      showNotification(`Пароль восстановлен. Вход выполнен ${mode}`, 'success');
    } catch (error) {
          ...
          <div className="login-form-container">
            ...
            <div style={{
              ...
              <p style={{
                ...
                lineHeight: '1.6',
                fontFamily: 'Inter, sans-serif'
              }}>Создайте аккаунт для доступа к виртуальному ассистенту тренировки презентаций</p>
            </div>
          ...
          <div className="product-showcase">
            ...
            <div className="product-description" style={{
              ...
              <p style={{ 
                ...
                Забудьте о статичных тренировках перед зеркалом. PitchForge создает динамичную 
                среду обучения, где ваш умный собеседник задает неожиданные вопросы, проверяет вашу 
                готовность к возражениям и помогает отработать сложные моменты презентации в режиме 
                реального времени.
              </p>
              ...
              <p style={{ 
                ...
                <strong style={{ fontWeight: '600' }}>
                  Просто отправьте ссылку на Zoom — и начинайте тренироваться!
                </strong>
              </p>
            </div>
          </div>
